...
class NMM_cross(nn.Module):
    """
    Nonlinear Multi-year Model that takes the follow-up time that is rounded to the closest year as input.
    The rounding operation happens in forward().
    ...
    def forward(self, features, missingness_mask, delta_t):
        ...
        """
        # Round delta_t to the nearest follow-up time.
        delta_t = torch.ceil(delta_t/12.0) * 12 
