...
class NMM_3layer(nn.Module):
    """
    Nonlinear Multi-year Model that has 2 hidden layers and 1 output layer.
    ...
    def __init__(self, input_size=114, model_width=128, model_depth=3, dropout_rate=0.2, num_classes=3):
        super(NMM_3layer, self).__init__()
        ...
        self.layers.append(nn.Linear(input_size, model_width))
        self.layers.append(nn.Linear(model_width, model_width))
        self.layers.append(nn.Linear(model_width, num_classes))
